...
public class OrderServiceImpl implements OrderService {
	...
	public OrderModel createOrder(Integer userId, Integer itemId, Integer promoId,Integer amount) throws BusinessException {
		...
		/*2.落单减库存(下单之前,锁定库存给当前用户订单使用,如果对应锁库存失败,就是说这个商品的库存不够了那就下单失败) 
		 * 支付减库存(不对库存进行锁定,无法保证用户不会超卖商家商品)这次不使用
		 */
	...
	@Transactional(propagation = Propagation.REQUIRES_NEW) //此方法事务一旦执行就不回滚了,无论外面的事务成功或失败
